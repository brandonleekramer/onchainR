...
- **JSON-RPC:** JSON-RPC APIs are the easiest way to get read data off of a blockchain. These APIs get basic account, balance, and network data. You can learn more about [blockchain JSON-RPC nodes here](https://www.alchemy.com/overviews/rpc-node).<br>
...
ether_balances = bind_rows(
  explorerBalances(my_wallet_list, "etherscan", etherscan_api_key) %>% mutate(chain = "Ethereum"),
  explorerBalances(my_wallet_list, "arbiscan", arbiscan_api_key) %>% mutate(chain = "Arbitrum"),
  explorerBalances(my_wallet_list, "basescan", arbiscan_api_key) %>% mutate(chain = "Base"),
  explorerBalances(my_wallet_list, "polygonscan", polygonscan_api_key) %>% mutate(chain = "Polygon")
)
