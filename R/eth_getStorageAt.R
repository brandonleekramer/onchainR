...
#'
#' eth_getStorageAt(vitalik_eth, my_provider_url, my_api_key)
#'
eth_getStorageAt = function(wallet_address, provider_url, api_key){
  request_output = httr2::request(
    stringr::str_c(provider_url, api_key)) %>% 
    httr2::req_headers('accept' = "application/json",
                'content-type' = "application/json") %>%
    httr2::req_body_raw(
    stringr::str_c('{"id": 1, "jsonrpc": "2.0", "method": "eth_getStorageAt",
                   "params": ["',wallet_address,'","0x0","latest"]}')) %>% 
    httr2::req_perform() %>%
